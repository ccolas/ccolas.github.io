...
import { VGDLSprite, OrientedSprite, OrientedFlicker } from "./vgdl-sprite.js";
import { SpriteProducer } from "./vgdl-sprite.js";
import { BASEDIRS, WHITE, GREEN } from "./constants.js";
...
export var MovingAvatar = function (gamejs, pos, size, args) {
...
MovingAvatar.prototype.update = function (game) {
	...
	if (action) this.physics.activeMovement(this, action, this.speed);
};
...
VerticalAvatar.prototype.update = function () {
	VGDLSprite.prototype.update.call(this, game);
	var action = this._readAction(game);
	if (action == UP || action == DOWN) this.physics.activeMovement(this, action);
};
/**
 *
 *
 *
 **/
export var FlakAvatar = function (gamejs, pos, size, args) {
...
OrientedAvatar.prototype.update = function (game) {
	...
	VGDLSprite.prototype.update.call(this, game);
	var action = this._readAction(game);
	if (action) this.physics.activeMovement(this, action);
...
export function ShootAvatar(gamejs, pos, size, args) {
	this.ammo = args.ammo;
...
ShootAvatar.prototype._hasAmmo = function (game) {
	...
	if (this.ammo in this.resources) return this.resources[this.ammo] > 0;
	return false;
};

ShootAvatar.prototype._reduceAmmo = function () {
	if (this.ammo && this.ammo in this.resources) this.resources[this.ammo]--;
};
...
ShootAvatar.prototype._shoot = function (game) {
	...
	if (this.stype && game.keystate[K_SPACE]) {
		...
		var newones = game._createSprite(
			[this.stype],
			[
				this.lastrect.left + u[0] * this.lastrect.size[0],
