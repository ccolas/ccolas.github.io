...
export function defaultDict(base) {
...
export var Tools = function () {
  ...
  that.triPoints = function (rect, orientation) {
    ...
    return [p1, p2a, p2b].map(function (p) {
      return [p[0], p[1]];
    });
  };
  ...
  that.Node = function (content, indent, parent = null) {
    ...
    that.indent = indent;
    if (parent) parent.insert(that);
    else that.parent = null;
    ...
    that.insert = function (node) {
      if (that.indent < node.indent) {
        if (that.children.length > 0)
          console.assert(
            that.children[0].indent == node.indent,
    ...
    that.getRoot = function () {
      if (that.parent) return that.parent.getRoot();
      else return that;
    };
  ...
  that.Node.prototype.toString = function () {
    if (this.children) return this.content;
    else return this.content + this.children.toString();
  };
