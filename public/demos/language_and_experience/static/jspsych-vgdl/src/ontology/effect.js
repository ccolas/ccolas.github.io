import { OrientedSprite, Resource } from "./vgdl-sprite.js";
import { BASEDIRS } from "./constants.js";
...
export var changeScore = scoreChange;
...
export function killSprite(sprite, partner, game, kwargs) {
	...
	if (partner != null) {
		...
		return ["killSprite", sprite.ID || sprite, null];
	}
}
...
export function turnAround(sprite, partner, game, kwargs) {
	...
	sprite.physics.activeMovement(sprite, DOWN, 1.0);
	sprite.lastmove = sprite.cooldown;
	sprite.physics.activeMovement(sprite, DOWN, 1.0);
...
function bounceDirection(sprite, partner, game, kwargs) {
	...
	var snorm = unitVector([
		-sprite.rect.centerx + partner.rect.centerx,
...
function wallBounce(sprite, partner, game, kwargs) {
	...
	stepBack(sprite, partner, game);
	if (
		Math.abs(sprite.rect.centerx - partner.rect.centerx) >
		Math.abs(sprite.rect.centery - partner.rect.centery)
	)
		sprite.orientation = (-sprite.orientation[0], sprite.orientation[1]);
...
export function killIfFromAbove(sprite, partner, game, kwargs) {
	if (
		sprite.lastrect.top > partner.lastrect.top &&
...
export function wrapAround(sprite, partner, game, kwargs) {
	...
	if (sprite.orientation[1] > 0) sprite.rect.top = offset * sprite.rect.size[1];
